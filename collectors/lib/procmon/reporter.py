...
class StdoutReporter:
	...
	#print reportlist
	for i in reportlist:
	    print '%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s'% (i['job_id'], i['task_id'], i['container_id'], str(i['cpu_user']), str(i['vmrss']), str(i['ivmss']), str(i['vmsize']), str(i['Xmx']), i['proc'], str(int(i['readbytes'])/1024), str(int(i['writebytes'])/1024))
    def endGroup(self):
        sys.stdout.flush()
